...
export const swipes = pgTable("swipe", {
  id: uuid("id").primaryKey().defaultRandom(),
  userId: text("user_id").notNull(),
  discoveryContentId: uuid("discovery_context_id").references(() => discoveryContent.id).notNull(),
  direction: swipeDirectionEnum("direction").notNull(),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [
  uniqueIndex("user_swipe_idx").on(table.userId, table.discoveryContentId),
]);