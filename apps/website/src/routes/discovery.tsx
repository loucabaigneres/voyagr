...
function DiscoveryPage() {
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      if (showResult) { if (e.key === 'Escape') setShowResult(false); return }
      if (e.key === 'ArrowRight' || e.key === 'l') commitSwipe(true)
      if (e.key === 'ArrowLeft' || e.key === 's') commitSwipe(false)
      if (e.key === ' ') { e.preventDefault(); triggerResult() }
    }
  ...
  function handleRestart() {
    setCursor(0); setLikes(0); setSkips(0); setHistory([])
    setShowResult(false); setDetail(null); saveTrip.reset()
  }
  ...
  return (
    <div className="fixed inset-0 z-[200] overflow-hidden bg-[#1a1a1a]">
