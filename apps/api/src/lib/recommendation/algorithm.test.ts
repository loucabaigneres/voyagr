...
import type { DiscoveryItem, SwipeRecord } from './algorithm.js';
...
function buildCatalog(): DiscoveryItem[] {
  ...
  for (let i = 0; i < 8; i++) items.push(makeItem('Rome', 'Italie', ['spa', 'terrasse']));
  for (let i = 0; i < 8; i++) items.push(makeItem('Paris', 'France', ['bar', 'restaurant']));
  for (let i = 0; i < 8; i++) items.push(makeItem('Berlin', 'Allemagne', ['piscine']));
